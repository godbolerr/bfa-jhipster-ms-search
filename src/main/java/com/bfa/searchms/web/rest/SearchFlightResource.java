package com.bfa.searchms.web.rest;
...
import com.bfa.searchms.web.rest.util.HeaderUtil;
...
public class SearchFlightResource {
	 ...
	 *            the searchFlightDTO to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new searchFlightDTO, or with status 400 (Bad Request) if the
	 *         searchFlight has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/search-flights")
	@Timed
	public ResponseEntity<SearchFlightDTO> createSearchFlight(@RequestBody SearchFlightDTO searchFlightDTO)
			throws URISyntaxException {
		log.debug("REST request to save SearchFlight : {}", searchFlightDTO);
		if (searchFlightDTO.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("searchFlight", "idexists",
					"A new searchFlight cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/search-flights/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("searchFlight", result.getId().toString())).body(result);
	}

	@PostMapping("/searchFlights")
	@Timed
	public ResponseEntity<List<SearchFlightDTO>> searchFlight(@RequestBody SearchFlightDTO searchFlightDTO)
			throws URISyntaxException {

		List<SearchFlightDTO> dtoList = searchFlightService.find(searchFlightDTO);
		
		
		return new ResponseEntity<List<SearchFlightDTO>>(dtoList, HttpStatus.OK);
	 ...
	 *
	 * @param searchFlightDTO
	 *            the searchFlightDTO to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         searchFlightDTO, or with status 400 (Bad Request) if the
	 *         searchFlightDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the searchFlightDTO couldnt be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/search-flights")
	@Timed
	public ResponseEntity<SearchFlightDTO> updateSearchFlight(@RequestBody SearchFlightDTO searchFlightDTO)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the searchFlightDTO to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         searchFlightDTO, or with status 404 (Not Found)
	 */
	...
	public ResponseEntity<SearchFlightDTO> getSearchFlight(@PathVariable Long id) {
		...
		return Optional.ofNullable(searchFlightDTO).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	 ...
	 *
	 * @param id
	 *            the id of the searchFlightDTO to delete
